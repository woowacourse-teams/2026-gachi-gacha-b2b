...
public class S3TransactionManager {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            ...
            public void afterCompletion(final int status) {
                if (status == TransactionSynchronization.STATUS_COMMITTED) {
                    moveImageToTrash(domainType, domainId, oldImageUrl);
                } else {
                    deleteImage(domainType, domainId, newImageUrl);
                }
            }
        });
    }
    ...
    public void trashImagesAfterRemoved(
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            ...
            public void afterCommit() {
                imageUrls.forEach(imageUrl -> moveImageToTrash(domainType, domainId, imageUrl));
            }
        });
    }
    ...
    public void deleteImagesAfterRemoved(
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            @Override
            public void afterCommit() {
                imageUrls.forEach(imageUrl -> deleteImage(domainType, domainId, imageUrl));
            }
        });
    }
    ...
    private void moveImageToTrash(final DomainType domainType, final Long domainId, final String imageUrl) {
        try {
            multipartUploader.moveToTrash(imageUrl);
        } catch (RuntimeException e) {
            log.error("{} 이미지를 휴지통으로 이동하는 데 실패했습니다. domainId={}, imageUrl={}",
                    domainType.getLabel(), domainId, imageUrl, e);
        }
    }

    private void deleteImage(final DomainType domainType, final Long domainId, final String imageUrl) {
        try {
